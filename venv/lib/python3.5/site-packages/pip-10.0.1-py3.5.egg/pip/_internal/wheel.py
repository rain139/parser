...
def move_wheel_files(name, req, wheeldir, user=False, home=None, root=None,
    ...
    def normpath(parser, p):
        return os.path.relpath(parser, p).replace(os.path.sep, '/')

    def record_installed(parserfile, destfile, modified=False):
        """Map archive RECORD paths to installation RECORD paths."""
        oldpath = normpath(parserfile, wheeldir)
    ...
    def clobber(source, dest, is_base, fixer=None, filter=None):
        ...
        for dir, subdirs, files in os.walk(source):
            ...
            for f in files:
                ...
                shutil.copyfile(parserfile, destfile)

                # Copy over the metadata for the file, currently this only
                # includes the atime and mtime.
                st = os.stat(parserfile)
                if hasattr(os, "utime"):
                    os.utime(destfile, (st.st_atime, st.st_mtime))

                # If our file is executable, then make our destination file
                # executable.
                if os.access(parserfile, os.X_OK):
                    st = os.stat(parserfile)
                    permissions = (
                ...
                record_installed(parserfile, destfile, changed)
