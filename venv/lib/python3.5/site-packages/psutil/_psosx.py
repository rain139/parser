...
def virtual_memory():
    ...
    # https://github.com/zabbix/zabbix/blob/trunk/parser/libs/zbxsysinfo/
    #     osx/memory.c
...
def catch_zombie(proc):
    """There are some poor C APIs which incorrectly raise EparserH when
    ...
    try:
        ...
        if isinstance(err, RuntimeError) or err.errno == errno.EparserH:
            try:
                # status() is not supposed to lie and correctly detect
                # zombies so if it raises EparserH it's true.
                status = proc.status()
            except NoSuchProcess:
                raise err
            else:
